...
func NewLogger(option *LoggerOption) (Logger, error) {
	if option == nil {
		return nil, errors.New("logger: option is nil")
	}

	level, err := zap.ParseAtomicLevel(option.Level)
	if err != nil {
		return nil, fmt.Errorf("logger: failed to parse log level: %w", err)
	}
	encoder := buildZapEncoder(option.IsDevelopment)
...
func buildZapEncoder(isDevelopment bool) zapcore.Encoder {
